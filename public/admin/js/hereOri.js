if (navigator.geolocation) {
	navigator.geolocation.getCurrentPosition(position => {
	 ...
	 // Initialize the platform object:
      ...
      // Marker di tambahkan setelah map berhasil di tampilkan
      function addDraggableMarker(map, behavior) {
        ...
        // Event Mendapatkan Data Pointer
      ...
      // Clear Space Hapus Marker Sebelumnya
      function clearSpace() {
      ...
      function calculateRouteAtoB(platform) {
        let router = platform.getRoutingService(),
            routerRequestParam = {
              ...
              waypoin0: urlParams.get('from'),
              waypoin1: urlParams.get('to')
            }
      ...
      function onSuccess(result) {
        ...
        addRouteShapeToMap(route); // Buat Jalan Di Map
        addSummaryToPanel(route.summary); // Summary = id di file show
      }
     ...
     function addRouteShapeToMap(route){
          ...
          // Membuat Garis-garis jalan
          routeShape.forEach(function(point) {
              let parts = point.split(',');
              linestring.pushLatLngAlt(parts[0], parts[1]);
          });

          // Titik Posisi - Lokasi Tujuan
          ...
          polyline = new H.map.Polyline(linestring, {
              ...
              lineTailCap: 'arrow-tail', // Memberikan Petunjuk Panah
  ...
  function openDirection(lat, lng, id) {
    window.open(`/show/${id}?from=${objLocalCoord.lat},${objLocalCoord.lng}&to=${lat},${lng}`, "_self");
  }
} else {
